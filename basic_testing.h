...
BT_POSSIBLY_UNUSED
static int bt_add_test(struct bt_test_descriptor ** tp) {
    (*tp)->next = 0;
    *bt_last_test_p = *tp;
    bt_last_test_p = &((*tp)->next);
    return 1;
}

#define TEST(test_name)							\
    static int test_name ## _fn ();					\
    static struct bt_test_descriptor test_name ## _descr				\
    = { # test_name, test_name ## _fn, __FILE__, __LINE__, 0};		\
    static struct bt_test_descriptor * test_name = & test_name ## _descr;		\
    static const int test_name ## _init = bt_add_test(& test_name);	\
    static int test_name ## _fn ()
...
static int bt_run_test(const struct bt_test_descriptor * t) {
	...
	if(WIFEXITED(exit_code)) {
	    ...
	    case SIGALRM: signame = "SIGALARM (timeout)"; break;
	    default: signame = 0;
	    }
	    if (bt_verbose) {
		if (signame)
